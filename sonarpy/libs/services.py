...
class ServiceIdentifier:
    ...
    TCP_SERVICES = {
        ...
        53: "domain", 80: "http", 110: "pop3", 111: "rpcbind", 119: "nntp",
        ...
        515: "printer", 520: "rip", 587: "submission", 631: "ipp",
        ...
        2222: "ssh-alt", 2375: "docker", 2376: "docker-ssl", 3000: "nodejs",
        3128: "squid", 3306: "mysql", 3389: "ms-wbt-server", 3690: "svn",
        4369: "epmd", 4443: "https-alt", 5000: "upnp", 5432: "postgresql",
        5672: "amqp", 5900: "vnc", 5901: "vnc-1", 5984: "couchdb",
        6379: "redis", 6443: "kubernetes", 6667: "irc", 7001: "weblogic",
        7002: "weblogic-ssl", 8000: "http-alt", 8008: "http-alt",
    ...
    UDP_SERVICES = {
        ...
        162: "snmptrap", 389: "ldap", 500: "isakmp", 514: "syslog",
        520: "rip", 1194: "openvpn", 1434: "ms-sql-m", 1604: "citrix",
        1701: "l2tp", 1812: "radius", 1813: "radius-acct", 1900: "upnp",
        2049: "nfs", 4500: "ipsec-nat-t", 5060: "sip", 5353: "mdns",
        5683: "coap", 6881: "bittorrent", 8125: "statsd", 9999: "abyss",
        10161: "snmptls", 33434: "traceroute",
    }

    TOP_PORTS_PRIORITY = [
        80, 443, 22, 21, 25, 53, 110, 143, 993, 995,
        3306, 3389, 8080, 445, 139, 135, 23, 8443, 587, 465,
        5432, 27017, 6379, 8000, 1433, 389, 636, 161, 162, 179,
        5900, 1080, 8888, 9090, 9200, 2049, 111, 5672, 15672, 11211,
        6443, 2375, 1883, 9042, 5984, 4369, 873, 514, 1521, 1723,
    ]

    @classmethod
    def get_service(cls, port: int, protocol: str = "tcp") -> str:
        if protocol.lower() == "tcp":
            return cls.TCP_SERVICES.get(port, "unknown")
        elif protocol.lower() == "udp":
            return cls.UDP_SERVICES.get(port, "unknown")
        return "unknown"
    ...
    @classmethod
    def get_top_ports(cls, n: int = 20) -> List[int]:
        top = cls.TOP_PORTS_PRIORITY[:n]
        if n > len(cls.TOP_PORTS_PRIORITY):
            remaining = sorted(
                p for p in cls.TCP_SERVICES.keys()
                if p not in cls.TOP_PORTS_PRIORITY
            )
            top.extend(remaining[: n - len(cls.TOP_PORTS_PRIORITY)])
        return sorted(top)
